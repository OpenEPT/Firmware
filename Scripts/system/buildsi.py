...
"""
@file    buildsys.py
@brief   Build EEPROM image from key:value parameters for device or charger.
"""
...
def generate_serial(params, target):
    now = datetime.now()
    year = now.year % 1000
    month = now.month
...
GENERATORS = {
    "HW_SER": lambda params, target: generate_serial(params, target),
    "MAC_ADDRESS": lambda params, target: generate_mac(),
}
...
def build_image(params, image_size):
...
def main():
    try:
        if len(sys.argv) != 5:
            print("Usage:")
            print("  python3 buildsys.py <device|charger> <params.txt> <image_size> <output.bin>")
            print("\nExamples:")
            print("  python3 buildsys.py device systemParams.txt 4096 system.bin")
            print("  python3 buildsys.py charger chargerParams.txt 128 charger.bin")
            return 1
        ...
        if target not in (TARGET_DEVICE, TARGET_CHARGER):
            log_err(f"Invalid target: {target}. Expected 'device' or 'charger'.")
            return 1

        try:
            image_size = int(sys.argv[3], 0)
        except ValueError:
            log_err(f"Invalid image size: {sys.argv[3]}")
            return 1

        log_step("INPUT VALIDATION")

        if not os.path.isfile(params_file):
            log_err(f"Parameter file not found: {params_file}")
            return 1
        ...
        log_ok(f"Target: {target}")
        log_ok(f"Parameter file: {params_file}")
        log_ok(f"Image size: {image_size} B")
        log_ok(f"Output: {output_file}")

        log_step("PARSE PARAMETERS")

        params = parse_params(params_file, target)

        if not params:
            log_err("No parameters found")
            return 1
        ...
        log_step("BUILD EEPROM IMAGE")

        image, payload_size, crc = build_image(params, image_size)
