/*
 * energy_debugger.h
 *
 *  Created on: Jun 23, 2024
 *      Author: Pavle Lakic & Dimitrije Lilic
 */

#ifndef CORE_MIDDLEWARES_SERVICES_ED_ENERGY_DEBUGGER_H_
#define CORE_MIDDLEWARES_SERVICES_ED_ENERGY_DEBUGGER_H_

#include <stdint.h>

#include "globalConfig.h"

#define ENERGY_DEBUGGER_TASK_NAME				CONF_ENERGY_DEBUGGER_TASK_NAME
#define ENERGY_DEBUGGER_TASK_PRIO				CONF_ENERGY_DEBUGGER_TASK_PRIO
#define ENERGY_DEBUGGER_STACK_SIZE				CONF_ENERGY_DEBUGGER_STACK_SIZE
#define	ENERGY_DEBUGGER_BUTTON_PORT				CONF_ENERGY_DEBUGGER_BUTTON_PORT
#define	ENERGY_DEBUGGER_BUTTON_PIN				CONF_ENERGY_DEBUGGER_BUTTON_PIN
#define ENERGY_DEBUGGER_BUTTON_ISR_PRIO			CONF_ENERGY_DEBUGGER_BUTTON_ISR_PRIO
#define	ENERGY_DEBUGGER_ID_QUEUE_LENTH			CONF_ENERGY_DEBUGGER_ID_QUEUE_LENGTH
#define	ENERGY_DEBUGGER_EBP_NAMES_QUEUE_LENTH	CONF_ENERGY_DEBUGGER_ID_QUEUE_LENGTH
#define	ENERGY_DEBUGGER_EBP_QUEUE_LENTH			CONF_ENERGY_DEBUGGER_EBP_QUEUE_LENGTH
#define	ENERGY_DEBUGGER_MESSAGE_BUFFER_LENTH	CONF_ENERGY_DEBUGGER_MESSAGE_BUFFER_LENTH
#define ENERGY_DEBUGGER_TCP_MESSAGE_BUFFER_LENTH CONF_ENERGY_DEBUGGER_TCP_MESSAGE_BUFFER_LENTH

#define ENERGY_DEBUGGER_MAX_CONNECTIONS		    CONF_ENERGY_DEBUGGER_MAX_CONNECTIONS

typedef enum
{
	ENERGY_DEBUGGER_STATE_INIT = 0,
	ENERGY_DEBUGGER_STATE_SERVICE,
	ENERGY_DEBUGGER_STATE_ERROR
}energy_debugger_state_t;

typedef enum
{
	ENERGY_DEBUGGER_STATUS_OK,
	ENERGY_DEBUGGER_STATUS_ERROR
}energy_debugger_status_t;

typedef struct
{
	uint8_t		serverIp[4];
	uint16_t	serverport;
	uint32_t	id;
}energy_debugger_connection_info;

energy_debugger_status_t ENERGY_DEBUGGER_Init(uint32_t timeout);
energy_debugger_status_t ENERGY_DEBUGGER_CreateLink(energy_debugger_connection_info* serverInfo, uint32_t timeout);

#endif /* CORE_MIDDLEWARES_SERVICES_ED_ENERGY_DEBUGGER_H_ */
